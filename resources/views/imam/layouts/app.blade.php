...
<head>
    ...
    <title>@yield('title', 'Imam Dashboard') - Masjid Connect</title>
    ...
    <div class="min-h-screen flex flex-col md:flex-row">
        ...
        <div id="sidebar" class="mobile-sidebar fixed md:sticky top-0 z-40 w-64 brand-gradient text-white h-screen md:h-screen py-6 px-4 flex-shrink-0 flex flex-col md:translate-x-0">
            <div class="mb-8 flex flex-col items-center text-center">
                ...
                <h1 class="text-2xl font-bold mt-2">Masjid Connect</h1>
                ...
                <div class="mt-2 text-xs px-3 py-1 bg-white/10 rounded-full">
                    <span>Imam Portal</span>
                </div>
            </div>
            
            <nav class="space-y-1.5">
                <a href="{{ route('imam.dashboard') }}" class="block py-2.5 px-4 rounded {{ request()->routeIs('imam.dashboard') ? 'bg-white/20 font-medium' : 'sidebar-item-hover transition' }}">
                ...
                <div class="py-2.5 px-4 mt-4 border-t border-green-600 pt-4 text-sm text-green-100 font-medium">
                    MASJID MANAGEMENT
                </div>
                
                <div x-data="{ open: false }" class="relative">
    <button @click="open = !open" class="w-full flex justify-between items-center py-2.5 px-4 rounded {{ request()->routeIs('imam.prayer-times*') ? 'bg-white/20 font-medium' : 'sidebar-item-hover transition' }}">
        <span><i class="fas fa-clock mr-2"></i> Prayer Times</span>
        <i class="fas" :class="open ? 'fa-chevron-up' : 'fa-chevron-down'"></i>
    </button>
    
    <div x-show="open" x-transition class="pl-4 space-y-1 mt-1">
        @foreach(auth()->user()->masjids as $masjid)
        <a href="{{ route('imam.prayer-times', $masjid->id) }}" class="block py-2 px-4 rounded text-sm {{ request()->is('imam/masjids/'.$masjid->id.'/prayer-times*') ? 'bg-white/10 font-medium' : 'sidebar-item-hover transition' }}">
            {{ $masjid->name }}
        </a>
        @endforeach
    </div>
                ...
                <div class="block py-2.5 px-4 rounded opacity-60">
                    <i class="fas fa-hand-holding-heart mr-2"></i> Donations
                    <span class="ml-2 px-1.5 py-0.5 bg-yellow-500 rounded-sm text-xs">Soon</span>
                </div>
                <div class="block py-2.5 px-4 rounded opacity-60">
        ...
        <div class="flex-1 flex flex-col md:ml-0">
            ...
            <header class="bg-white shadow-sm py-4 px-6 flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-gray-200">
                <div class="flex items-center">
                    <h2 class="text-xl font-semibold text-gray-800">@yield('header', 'Imam Dashboard')</h2>
                </div>
