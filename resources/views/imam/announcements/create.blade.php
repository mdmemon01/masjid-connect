...
@section('header', 'Create New Announcement')
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
    <h2 class="text-lg font-semibold text-gray-800 mb-6">Create Announcement for {{ $masjid->name }}</h2>
    
    <form action="{{ route('imam.announcements.store', $masjid->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mb-6">
            ...
            <p class="mt-1 text-xs text-gray-500">Recommended size: 800x400 pixels (Max 2MB)</p>
            @error('image')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            <label class="inline-flex items-center">
                <input type="checkbox" name="is_published" class="rounded border-gray-300 text-green-600 shadow-sm focus:border-green-300 focus:ring focus:ring-green-200 focus:ring-opacity-50" {{ old('is_published', true) ? 'checked' : '' }}>
